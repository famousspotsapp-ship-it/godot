...
For detailed technical documentation — entity hierarchy, signal flow, procedural
level generation, collision layers, and how to add new content — see
[ARCHITECTURE.md](ARCHITECTURE.md).

## Key Design Decisions

- **Procedural level**: The entire tilemap is built in `level_1.gd` at runtime
  from constant arrays. No hand-authored tilemap data in the `.tscn` file.
- **Programmatic assets**: All sprites and audio are generated by Python
  (`tools/gen_assets.py`), making the pipeline fully reproducible.
- **Autoload singleton**: `GameManager` centralises run state (lives, score, HP)
  and all scene transitions, so gameplay scripts never call
  `change_scene_to_file()` directly.
- **Entity base class**: All enemies inherit `enemy_base.gd`, which provides HP,
  damage, hit-flash, and death logic. Subclasses only override behaviour.
- **Animation helper**: `anim_util.gd` builds `SpriteFrames` from horizontal
  sprite-sheet strips, keeping animation declarations declarative.
